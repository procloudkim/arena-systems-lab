...
- Status: Accepted
- Date: 2026-09-04
- 문서 파일명·비기술적 표현 정정: [ADR 0012](0012-technical-documentation-governance.md), 기술 결정과 당시 검증은 유지

## Context

현재 저장소는 Unity 기반 플레이 가능한 vertical slice, OOP 책임 분리, Enemy FSM, Git 운영 근거를 갖고 있다. 프로젝트의 최종 하한에는 Unity, Unreal Engine, Git, Apache Subversion(SVN), MySQL, network programming, socket programming, multithreading, OOP가 모두 실제 검증 근거와 함께 포함돼야 한다.
...
공개 운영과 다중 사용자 게임 시뮬레이션은 이번 기술 확장 범위에서 제외한다.
...
5. 후속 milestone에서 MySQL 8.4 LTS에 run과 score를 저장한다. 이는 승인 대기 계획이며 현재 저장 구현이 아니다. schema와 migration SQL은 추적하지만 credential과 local data volume은 추적하지 않는다.
...
MySQL wire protocol은 직접 구현하지 않는다. 2026-09-04 감사 당시 connector 후보는 `MySqlConnector` 2.6.2이며 추가 전 승인이 필요하다. 대안은 Oracle `MySql.Data` 9.7.0이다. SVN은 현재 미설치이므로 해당 milestone은 승인 전까지 시작할 수 없다.

공개 배포, authentication, cloud hosting, multiplayer simulation은 이 결정의 범위가 아니다. 최초 network 통합은 loopback에서만 검증한다.
...
- Remote branch SHA: PASS, commit `01cd869`가 당시 원격 작업 branch과 일치
